-- Fix a latent bug in hard_delete_organization() from 20260909020000: the orphaned-member
-- guards referenced `demo_scenarios.last_run_by`, but that column lives on `qa_test_cases`
-- (demo_scenarios has no member FK). plpgsql bodies aren't checked until runtime, so the
-- function was created fine but would ERROR ("column x.last_run_by does not exist") the next
-- time an org was deleted. Redefine with the correct table.
