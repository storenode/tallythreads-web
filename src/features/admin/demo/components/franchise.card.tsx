...
type AgreementStatus = "Active" | "Upcoming" | "Expired";

/** Derive an agreement's status from its dates against today. */
...
const STATUS_TONE: Record<AgreementStatus, "green" | "blue" | "amber"> = {
  Active: "green",
  Upcoming: "blue",
  Expired: "amber",
};
...
export default function FranchiseCard({
  ...
  const { data: group } = useFranchiseGroupByOrg(organization.id);
  const { data: memberships } = useFranchiseMemberships(
    organization.id,
    group?.id,
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge>Registration · Franchise</Badge>
            <Badge tone="green">
      ...
      {/* About the Franchise registration type */}
      {showAbout && (
        <div>
          <p
          ...
          >
            ...
            registration groups stores under a franchisor brand, where each store
            operates under an{" "}
            <span className="font-semibold text-gray-800 dark:text-white/90">
              agreement
            </span>{" "}
            (with start and end dates) rather than being centrally owned like a
            chain. Here,{" "}
            <span className="font-semibold text-gray-800 dark:text-white/90">
              {organization.legal_name || organization.name}
            </span>{" "}
            is the franchisor, and each store below is linked to its franchise
            group for an agreed period.
          </p>
          <p
            className="mt-3 text-[13px] leading-relaxed text-gray-400 dark:text-gray-500"
            style={{ fontFamily: SANS }}
          >
            In TallyThreads, a store is treated as "franchise" only when it has an
            active franchise agreement — not just because of a label. Settlement /
            royalty terms are not computed yet; only the agreement linkage is shown
            here.
          </p>
        </div>
      )}
      ...
      {/* Franchise */}
      <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-white/[0.02]">
        <SectionHeading icon={<Network size={16} />}>Franchise</SectionHeading>
        <dl className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          <Field
            label="Franchisor"
            value={organization.legal_name || organization.name}
          />
          <Field label="Franchise group" value={group?.name} />
        </dl>
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p
                        ...
                        style={{ fontFamily: MONO }}
                      >
                        {m.agreement_start} → {m.agreement_end ?? "Active"}
                      </p>
                    </div>
