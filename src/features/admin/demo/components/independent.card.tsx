...
interface LaunchProps {
  /** Issue + copy a launch link for this member. */
  onCopyLink?: (memberId: string) => void;
  /** memberId whose link is currently being issued (shows a spinner). */
  busyId?: string;
  /** memberId whose link was just copied (shows "Copied!"). */
  copiedId?: string;
}
...
function StoreBlock({
  ...
  // The org store list is minimal (id/name/code); fetch the full row for the
  // hours/address/gstin details shown below.
...
export default function IndependentCard({
  organization,
}: {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <Badge>Registration · Individual</Badge>
            <Badge tone="green">
      ...
      {/* About the Individual registration type */}
      {showAbout && (
        <div className="rp-4">
          <p
          ...
          >
            ...
            registration is for a cloth store run and owned by a single person
            rather than a partnership or a registered company. Here,{" "}
            <span className="font-semibold text-gray-800 dark:text-white/90">
              {organization.legal_name || organization.name}
            </span>{" "}
            trades under the owner's own legal name — the simplest setup for a
            proprietor-run outlet, with no separate legal entity, shared
            ownership, or corporate paperwork behind it.
          </p>
          <p
            className="mt-3 text-[13px] leading-relaxed text-gray-400 dark:text-gray-500"
            style={{ fontFamily: SANS }}
          >
            In TallyThreads, the organization name is the legal entity, and
            every organization has one or more stores under it — sometimes just
            a single store, in which case the organization name and the store
            name are the same. Either way, to work with TallyThreads an
            organization and at least one store must both be registered.
          </p>
        </div>
      )}
