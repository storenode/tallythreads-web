...
const VARIANT_CLASSES = {
  default: "focus:border-brand focus:ring-brand/10",
  error: "border-error-text focus:border-error-text focus:ring-error-text/10 pr-10",
  success:
    "border-success-text focus:border-success-text focus:ring-success-text/10 pr-10",
} as const;

const HINT_CLASSES = {
  default: "mt-1.5 text-xs text-fg-muted",
  error: "mt-1.5 text-xs text-error-text",
  success: "mt-1.5 text-xs text-success-text",
} as const;
...
interface InputProps extends InputHTMLAttributes<HTMLInputElement> {
  ...
  hint?: ReactNode;
  /** Fixed text/element pinned to the right edge of the field, e.g. "@gmail.com". */
  suffix?: ReactNode;
}

export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  { label, error, success, hint, suffix, className = "", id, type = "text", ...rest },
  ref,
) {
  const autoId = useId();
  const inputId = id || autoId;

  const variant: Variant = error ? "error" : success ? "success" : "default";
  const message =
    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          className={`${BASE_CLASSES} ${VARIANT_CLASSES[variant]} ${suffix ? "pr-28" : ""} ${className}`}
          {...rest}
        />

        {suffix && (
          <span className="pointer-events-none absolute top-1/2 right-3.5 -translate-y-1/2 text-sm text-fg-muted">
            {suffix}
          </span>
        )}
      ...
      {message && <p className={HINT_CLASSES[variant]}>{message}</p>}
    </div>
  );
});
