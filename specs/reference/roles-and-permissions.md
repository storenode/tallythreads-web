...
| `inventory.write` / `inventory.read` | Inventory (M3) | **In use (2026-09-26)** — gate `stock_locations` (§3B) and `inventory_categories` (§3D) RLS via `has_store_permission`. Product master/items still to build | `org_owner`, `org_manager`, `store_sales_staff`, `store_temp_staff` |
