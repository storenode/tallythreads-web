# TallyThreads — Multi-Store-Model Master Plan
...
|---|---|---|
| **`organizations`** | The top-level billing/subscription entity — "who TallyThreads has a customer relationship with." An independent owner, a chain owner, a franchisor brand, a distributor's business are all just an `organization`. | All 8 |
...
internal movement with no payment attached. A franchise's stores are a *separate*
...
₹2,500 flips the *whole* piece from 18% to 5%, exactly as `gstCalc.ts` already models
